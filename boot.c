...
static void check_backup_boot(DOS_FS * fs, struct boot_sector *b, unsigned int lss)
{
    ...
    if (!fs->backupboot_start) {
        ...
        if (!interactive || GetKey("12", "?") == '1') {
            ...
            FsWrite(fs, fs->backupboot_start, sizeof(*b), b);
            FsWrite(fs, offsetof(struct boot_sector, backup_boot),
    ...
    FsRead(fs, fs->backupboot_start, sizeof(b2), &b2);
    if (memcmp(b, &b2, sizeof(b2)) != 0) {
        ...
        switch (interactive ? GetKey("123", "?") : '3') {
        case '1':
            FsWrite(fs, fs->backupboot_start, sizeof(*b), b);
            break;
        case '2':
            FsWrite(fs, 0, sizeof(b2), &b2);
            break;
        default:
            break;
        }
...
static void read_fsinfo(DOS_FS * fs, struct boot_sector *b, unsigned int lss)
{
    ...
    if (!b->info_sector) {
        ...
        if (interactive && GetKey("12", "?") == '1') {
            ...
            if (s > 0 && s < le16toh(b->reserved)) {
                init_fsinfo(&i);
                FsWrite(fs, (off_t)s * lss, sizeof(i), &i);
                b->info_sector = htole16(s);
                FsWrite(fs, offsetof(struct boot_sector, info_sector),
                          sizeof(b->info_sector), &b->info_sector);
                if (fs->backupboot_start)
                    FsWrite(fs, fs->backupboot_start +
    ...
    FsRead(fs, fs->fsinfo_start, sizeof(i), &i);
    ...
    if (i.magic != htole32(0x41615252) ||
        ...
        if (!interactive || GetKey("12", "?") == '1') {
            init_fsinfo(&i);
            FsWrite(fs, fs->fsinfo_start, sizeof(i), &i);
        } else
            fs->fsinfo_start = 0;
    }
...
void BootRead(DOS_FS * fs)
{
    ...
    FsCheck(fs, (off_t)((total_sectors & ~1) - 1) * logical_sector_size,
              logical_sector_size);
...
static void write_boot_label(DOS_FS * fs, char *label)
{
    if (fs->fat_bits == 12 || fs->fat_bits == 16) {
        ...
        FsWrite(fs, 0, sizeof(b), &b);
        if (fs->backupboot_start)
            FsWrite(fs, fs->backupboot_start, sizeof(b), &b);
    }
}

static off_t find_volume_de(DOS_FS * fs, DIR_ENT * de)
{
    uint32_t cluster;
    off_t offset;
    int i;
    ...
    if (fs->root_cluster) {
        for (cluster = fs->root_cluster;
            ...
            for (i = 0; i * sizeof(DIR_ENT) < fs->cluster_size; i++) {
                FsRead(fs, offset, sizeof(DIR_ENT), de);
...
static void write_volume_label(DOS_FS * fs, char *label)
{
    ...
    if (offset == 0) {
        created = 1;
        offset = AllocRootEntry(fs, &de, label, 0);
    }
    ...
    FsWrite(fs, offset, sizeof(DIR_ENT), &de);
}

void LabelWrite(DOS_FS * fs, char *label)
{
    int l = strlen(label);
